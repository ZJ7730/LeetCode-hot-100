...
`dp[i][0]` 表示必须以 `nums[i]` 结尾的连续子数组最大乘积；`dp[i][1]` 表示必须以 `nums[i]` 结尾的连续子数组最小乘积。
...
`dp[i][0]` 表示第 `i` 天结束时持有股票的最大利润；`dp[i][1]` 表示第 `i` 天结束时不持有股票的最大利润。

```java
class Solution {

    /**
    * dp[i][0] 表示第 i 天持股的最大利润
    * dp[i][1] 表示第 i 天不持股的最大利润
    */
...
`dp[i][0]` 表示持股，`dp[i][1]` 表示自由空仓，`dp[i][2]` 表示今天卖出，`dp[i][3]` 表示今天处于冷冻期；所有值都是第 `i` 天结束时的最大利润。
...
`dfs(root)` 返回长度为 `2` 的数组：`dp[0]` 表示不抢当前根节点时该子树的最大金额，`dp[1]` 表示抢当前根节点时该子树的最大金额。
